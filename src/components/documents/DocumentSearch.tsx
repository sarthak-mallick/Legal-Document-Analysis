    ...
    <div className="space-y-3">
      <form onSubmit={handleSearch} className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-border bg-card-bg px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
        />
        <button
          type="submit"
      ...
      {results.length > 0 && (
        <div className="space-y-3">
          {results.map((doc) => (
            <div key={doc.documentId} className="rounded-lg border border-border bg-card-bg p-4">
